...
uploaded_image = st.sidebar.file_uploader('Here',type=['jpeg','jpg','png','webp'])
if uploaded_image:
    image = Image.open(uploaded_image) #Converts the uploaded file into a PIL Image object
    st.sidebar.subheader(':blue[UPLOADED IMAGE]')
    st.sidebar.image(image)

# It creates an image upload section in the Streamlit sidebar,
# lets the user upload an image,
# opens the image using Pillow,
# and displays the uploaded image back to the user.

# Create main page
st.title(':orange[STRUCTURAL DEFFECTS] : :green[AI assisted structural defect identifier in construction business]')
tips ='''To use the application, follow these steps:
* Upload the image
...
of a structure. You need to identify the strucutural defects in the image and generate
...
* Make sure the remedial measures are not missed in tyring to make the report concise.
* provide a shot term and long solution along with their estimated cost (in INR) and time to implement.
...
* Make sure that the report does not exceeds 3 pages and yet covers all points.
'''
